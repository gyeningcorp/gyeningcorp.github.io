...
A single-page web app for finding small local bands playing at bars and restaurants **tonight**, starting with Pittsburgh and expanding globally via discovery links.
...
- 🎤 **Venue directory** — every venue is tagged **Scraped**, **Social Watch**, or **Linked Only** so users can see how each listing is sourced.
...
GitHub Pages is static-only. The client-side scraper uses public CORS proxies, which are rate-limited and unreliable — expect the status bar to show red pills on some venues. This is exactly the honest "Scraped / Social Watch / Linked Only" model from the spec.
...
├── privacy-policy.html   # required for App Store / Play Store later
├── 404.html
└── README.md
```
...
- Migrate scraping to Vercel serverless (`api/events.js`) when a real domain is bought
